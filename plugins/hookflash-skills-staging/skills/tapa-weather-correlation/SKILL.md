...
- **Use your Tether MCP connector.** The `tapa_wc_*` tools are limited to a small allow-list of
  ...
  - If other Tether tools are available but the `tapa_wc_*` tools are missing, the user is not on
...
- **If a `tapa_wc_*` call fails with an authentication or authorisation error from Tapa**, the
