...
- **Use your Tether MCP connector.** The `tapa_da_*` tools are limited to a small allow-list of
  ...
  - If other Tether tools are available but the `tapa_da_*` tools are missing, the user is not on
...
- **If a `tapa_da_*` call fails with an authentication or authorisation error from Tapa**, the
